...
def get_scaler(file_train, use_seq_dim: list, use_meta_dim: tuple, use_app_dim: list, mode='seq'):
    ...
    elif mode == 'fft_sort' or mode == 'fft_idx' or mode == 'token_int':
        train_X_mode_cut = []
        for line in train_X_mode:
...
class Collate:
    ...
    def _collate(self, batch):
        """
        args:
            # 仅包长时间序列 batch - list of (tensor, label)
            引入属性 batch - list of (X_seq, X_meta, label)
...
def get_data_loader(device, seq_len_max, mode, use_seq_dim: list, use_meta_dim: tuple, use_app_dim: list):
    ...
    # val_abu_ds = NetFlowDataset(mode, 'test_abu', scaler_seq, scaler_meta, scaler_token, device, seq_len_max, use_seq_dim, use_meta_dim, use_app_dim)
    # val_drb_ds = NetFlowDataset(mode, 'test_drb', scaler_seq, scaler_meta, scaler_token, device, seq_len_max, use_seq_dim, use_meta_dim, use_app_dim)
    # val_jm_ds = NetFlowDataset(mode, 'test_jm', scaler_seq, scaler_meta, scaler_token, device, seq_len_max, use_seq_dim, use_meta_dim, use_app_dim)
    # # val_c2_ds = NetFlowDataset(mode, 'test_c2', scaler_seq, scaler_meta, scaler_token, device, seq_len_max, use_seq_dim, use_meta_dim, use_app_dim)
    # val_wb_ds = NetFlowDataset(mode, 'test_wb', scaler_seq, scaler_meta, scaler_token, device, seq_len_max, use_seq_dim, use_meta_dim, use_app_dim)
    # train_ds 读所有数据; Collate 把读入的数据处理成模型输入
    ...
    # # val_c2_dl = data.DataLoader(val_c2_ds, batch_size=BATCH_SIZE, shuffle=False, collate_fn=Collate(device))
    ...
    return dim_seq, dim_meta, dim_token, train_dl, val_dl, test_dl
    # return dim_seq, dim_meta, dim_token, train_dl, val_dl, test_dl, val_abu_dl, val_drb_dl, val_jm_dl, val_wb_dl
    # dim_seq, dim_meta, dim_fft_sort, dim_fft_idx = train_ds.get_dim()
    # return dim_seq, dim_meta, dim_fft_sort, dim_fft_idx, train_dl, val_dl, test_dl, val_abu_dl, val_drb_dl, val_jm_dl, val_c2_dl, val_wb_dl
